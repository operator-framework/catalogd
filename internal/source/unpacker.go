...
type Unpacker interface {
	Unpack(context.Context, *catalogdv1beta1.CatalogSource) (*Result, error)
}
...
func (s *unpacker) Unpack(ctx context.Context, catalog *catalogdv1beta1.CatalogSource) (*Result, error) {
...
func NewDefaultUnpacker(systemNsCluster cluster.Cluster, namespace, unpackImage string) (Unpacker, error) {
	...
	return NewUnpacker(map[catalogdv1beta1.SourceType]Unpacker{
		...
		catalogdv1beta1.SourceTypeGit: &Git{
			Reader:          systemNsCluster.GetClient(),
			SecretNamespace: namespace,
		},
		catalogdv1beta1.SourceTypeConfigMaps: &ConfigMaps{
			Reader:             systemNsCluster.GetClient(),
			ConfigMapNamespace: namespace,
		},
		catalogdv1beta1.SourceTypeHTTP: &HTTP{
			Reader:          systemNsCluster.GetClient(),
			SecretNamespace: namespace,
		},
	}), nil
