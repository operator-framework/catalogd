...
func (r *Git) Unpack(ctx context.Context, catalog *catalogdv1beta1.CatalogSource) (*Result, error) {
	if catalog.Spec.Source.Type != catalogdv1beta1.SourceTypeGit {
		return nil, fmt.Errorf("catalog source type %q not supported", catalog.Spec.Source.Type)
	}
	if catalog.Spec.Source.Git == nil {
		return nil, fmt.Errorf("catalog source git configuration is unset")
	}
	gitsource := catalog.Spec.Source.Git
	if gitsource.Repository == "" {
		// This should never happen because the validation webhook rejects git catalogs without repository
	...
	cloneOpts := git.CloneOptions{
		...
		Progress:        &progress,
		Tags:            git.NoTags,
		InsecureSkipTLS: catalog.Spec.Source.Git.Auth.InsecureSkipVerify,
	}

	if catalog.Spec.Source.Git.Auth.Secret.Name != "" {
		auth, err := r.configAuth(ctx, catalog)
		if err != nil {
	...
	if err != nil {
		return nil, fmt.Errorf("catalog unpack git clone error: %v - %s", err, progress.String())
	}
	wt, err := repo.Worktree()
	if err != nil {
		return nil, fmt.Errorf("catalog unpack error: %v", err)
	}
	...
	var catalogFS fs.FS = &billyFS{wt.Filesystem}
	...
	resolvedGit := catalog.Spec.Source.Git.DeepCopy()
	resolvedGit.Ref = catalogdv1beta1.GitRef{
		Commit: commitHash.String(),
	}

	resolvedSource := &catalogdv1beta1.CatalogSourceSource{
		Type: catalogdv1beta1.SourceTypeGit,
		Git:  resolvedGit,
	}
...
func (r *Git) configAuth(ctx context.Context, catalog *catalogdv1beta1.CatalogSource) (transport.AuthMethod, error) {
	var auth transport.AuthMethod
	if strings.HasPrefix(catalog.Spec.Source.Git.Repository, "http") {
		userName, password, err := r.getCredentials(ctx, catalog)
		if err != nil {
			return nil, err
		}
		return &http.BasicAuth{Username: userName, Password: password}, nil
	}
	privatekey, host, err := r.getCertificate(ctx, catalog)
	if err != nil {
		return nil, err
	}
	...
	if catalog.Spec.Source.Git.Auth.InsecureSkipVerify {
		auth = &sshgit.PublicKeys{
			User:   "git",
			Signer: signer,
...
func (r *Git) getCredentials(ctx context.Context, catalog *catalogdv1beta1.CatalogSource) (string, string, error) {
	secret := &corev1.Secret{}
	err := r.Get(ctx, client.ObjectKey{Namespace: r.SecretNamespace, Name: catalog.Spec.Source.Git.Auth.Secret.Name}, secret)
	if err != nil {
		return "", "", err
	}
...
// getCertificate reads certificate from the secret specified in the catalog
// It returns the privatekey and the entry of the host in known_hosts when they are in the secret
func (r *Git) getCertificate(ctx context.Context, catalog *catalogdv1beta1.CatalogSource) ([]byte, []byte, error) {
	secret := &corev1.Secret{}
	err := r.Get(ctx, client.ObjectKey{Namespace: r.SecretNamespace, Name: catalog.Spec.Source.Git.Auth.Secret.Name}, secret)
	if err != nil {
		return nil, nil, err
